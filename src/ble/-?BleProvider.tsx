...
export const BleProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
	...
	//Backoff/Retry on disconnect
	useEffect(() => {
		const manager = managerRef.current;
		const MAX_RECONNECT_ATTEMPTS = 5;
		const INITIAL_BACKOFF_MS = 1000; // 1 second

		Object.values(connectedRef.current).forEach(({ device }) => {
			let reconnectAttempts = 0;

			const tryReconnect = async (disconnectedDevice) => {
				let backoff = INITIAL_BACKOFF_MS;
				while (reconnectAttempts < MAX_RECONNECT_ATTEMPTS) {
					try {
						await connectToDevice(disconnectedDevice);
						console.log('Reconnected to device:', disconnectedDevice.id);
						reconnectAttempts = 0; // Reset on success
						break;
					} catch (e) {
						reconnectAttempts++;
						console.warn(`Reconnection attempt ${reconnectAttempts} failed for device ${disconnectedDevice.id}:`, e);
						await new Promise((res) => setTimeout(res, backoff));
						backoff *= 2; // Exponential backoff
					}
				}
				if (reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
					console.warn(`Max reconnection attempts reached for device ${disconnectedDevice.id}.`);
				}
			};

			const sub = manager.onDeviceDisconnected(device.id, async (error, disconnectedDevice) => {
				if (error) {
					console.warn('Device disconnect error:', error);
					return;
				}
				console.log('Device disconnected:', disconnectedDevice.id);
				await tryReconnect(disconnectedDevice);
			});
			// Clean up subscription on unmount or device change
			return () => sub.remove();
		});
	}, [connected]);
