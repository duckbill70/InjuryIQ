...
function findBatteryCharacteristic(
	entry: ConnectedDeviceLike,
): Characteristic | null {
	...
	const char = list.find((c) =>
		BATTERY_CHAR_UUIDS.some((want) => uuidsEqual(c.uuid, want)),
	);
	return char ?? null;
}
...
export function useBatteryPercent(
	...
	useEffect(() => {
		charRef.current = batteryChar ?? null;
	}, [batteryChar?.uuid, entry?.id]);
