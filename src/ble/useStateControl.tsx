...
function findStateChar(
	entry?: ConnectedDeviceLike | null,
): Characteristic | null {
	if (!entry) return null;
	const svcKey =
		entry.services.find((s) => s.toLowerCase() === STATE_SERVICE_UUID) ??
		STATE_SERVICE_UUID;
	const list =
...
export function useStateControl(
	...
	useEffect(() => {
		charRef.current = stateChar ?? null;
	}, [stateChar?.uuid, entry?.id]);
