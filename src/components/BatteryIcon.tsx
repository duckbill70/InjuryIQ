...
import { Battery as BatteryEmpty, BatteryCharging, BatteryFull, BatteryMedium, BatteryLow } from 'lucide-react-native';
...
const BatteryIconComponent: React.FC<BatteryIconProps> = ({ level = 0, charging = false, size = 30, color, placeholderColor, vertical = false, style, accessibilityLabel }) => {
	...
	const a11yLabel = accessibilityLabel ?? (isPlaceholder ? 'Battery unavailable' : charging ? `Battery charging at ${lvl} percent` : `Battery ${lvl} percent`);

	return (
		<View accessible accessibilityLabel={a11yLabel} style={[{ width: size, height: size, alignItems: 'center', justifyContent: 'center' }, vertical && { transform: [{ rotate: '-90deg' }] }, style]}>
			{renderIcon()}
		</View>
	);
};
