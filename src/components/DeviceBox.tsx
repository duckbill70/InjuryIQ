...
const SERVICES: Record<
  ServiceKey,
  { label: string; Icon: IconType; serviceUuids: string[]; charUuids: string[] }
> = {
  fatigue: {
    label: 'Fatigue',
    Icon: Activity,
...
function SmallBtn ({label, onPress}: {label: string; onPress: () => void}) {
  return(
    <Text onPress={onPress} style={{paddingHorizontal: 8, paddingVertical: 4, backgroundColor: '#1e90ff', borderRadius: 6}}>
      {label}
    </Text>
  )
}
...
function stateModeToTint(mode: number | null | undefined, fallback: string) {
  switch (mode) {
    case StateMode.Amber:     return '#FFB300'; // amber
    case StateMode.RedPulse:  return '#EF4444'; // red-500
    case StateMode.Red:       return '#EF4444'; // red-500
    case StateMode.GreenPulse:return '#22C55E'; // green-500
    case StateMode.Green:     return '#22C55E'; // green-500
    case StateMode.BluePulse: return '#3B82F6'; // blue-500
    case StateMode.Blue:      return '#3B82F6'; // blue-500
    case StateMode.Off:       return '#9CA3AF'; // gray-400
    default:                  return fallback;  // when unknown
  }
}


export default function DeviceBox({ item, height = 150 }: Props) {
  ...
  //State Control
  const { value: valueState, supported: supportedState, error: errorState, commands, setStateMode } = useStateControl(item, {subscribe : true, intervalMs: 0})
  ...
  return (
    <View style={[theme.viewStyles.card, {  flex: 0.48, flexDirection: 'column', backgroundColor: 'white', opacity: 0.8 }]}>
      {/* Title */}
      ...
      <View style={{alignItems: 'center', marginTop: 6}}>
          <Text style={theme.textStyles.body2}>
            Battery: {supportedBat ? (percent != null ? `${percent}%` : '-') : 'N/A' }
          </Text>
          {!!errorBat && <Text style={[theme.textStyles.body2, {fontSize: 8, color: 'tomato'}]}>{errorBat}</Text>}
      </View>

      {/* State */}
      <View style={{ borderTopWidth: 1, borderTopColor: border, paddingTop: 6, paddingBottom: 6 }}>
      ...
      <View style={{ borderTopWidth: 1, borderTopColor: border, paddingTop: 6 }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center' }}>
          {serviceStates.map(({ key, label, Icon, present, active }) => {
            let iconColor = present ? (active ? primary : muted) :border
            ...
            <View key={key} style={{ alignItems: 'center', minWidth: 44 }}>
              ...
              <Text
                style={[
                  ...
                  { marginTop: 2, color: present ? (active ? primary : muted) : border },
                ]}
              >
                {label}
              </Text>
