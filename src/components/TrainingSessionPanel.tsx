...
const TrainingSessionPanelComponent: React.FC = () => {
	...
	const [snapshotStatus, setSnapshotStatus] = useState<Record<string, { slots: boolean[] }>>({});
		...
		<View style={[theme.viewStyles.panelContainer, { backgroundColor: theme.colors.black, borderWidth: 1, borderColor: theme.colors.white, marginBottom: 24, padding: 20 }]}>
			...
			<View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 18 }}>
				...
				<Pressable
					...
					disabled={sessionActive || !leftDevice || deviceState[leftDevice.id] !== ControlState.STOPPED}
					style={({ pressed }) => [
						{
							marginLeft: 12,
				...
				<Pressable
					...
					disabled={sessionActive || !rightDevice || deviceState[rightDevice.id] !== ControlState.STOPPED}
					style={({ pressed }) => [
						{
							marginLeft: 12,
							paddingHorizontal: 12,
			...
			<View style={{ flexDirection: 'column', marginBottom: 12 }}>
				{[0, 1, 2].map((idx) => {
					// Aggregate snapshot status for this interval across both devices
					const leftSlots = leftDevice ? snapshotStatus[leftDevice.id]?.slots || [false, false, false] : [false, false, false];
					const rightSlots = rightDevice ? snapshotStatus[rightDevice.id]?.slots || [false, false, false] : [false, false, false];
					const slotFilled = leftSlots[idx] || rightSlots[idx];
									...
									return (
										<View
											...
											style={{
												width: 5,
												height: 20,
												backgroundColor: filled ? theme.colors.white : theme.colors.muted,
			...
			<View style={{ marginBottom: 12 }}>
				<Text style={[theme.textStyles.body2, { color: theme.colors.white }]}>
					Current Interval: {currentIntervalIdx + 1} / {intervals.length}
				</Text>
				...
				{connectedDevices.map((d) => (
					<View key={d.id} style={{ marginBottom: 2 }}>
						<Text style={[theme.textStyles.body2, { color: theme.colors.white }]}>
							{d.name || d.id.slice(-6)} — FIFO: {fifoPct[d.id] !== undefined && fifoPct[d.id] !== null ? `${fifoPct[d.id]}%` : '—'} | State:{' '}
							{deviceState[d.id] !== undefined && deviceState[d.id] !== null ? ControlState[deviceState[d.id]!] : '—'}
						</Text>
