...
export type CombinedWriter = {
	...
	onBatchA(batch: RawPacket[] | undefined | null, fatigueValue?: number | null): void;

	/** Append batch from device B. */
	onBatchB(batch: RawPacket[] | undefined | null, fatigueValue?: number | null): void;
...
export function createCombinedImuWriter(opts: FactoryOptions): CombinedWriter {
	...
	function appendRows(src: 'A' | 'B', id: string | null, batch?: RawPacket[] | null, fatigueValue?: number | null) {
		...
		for (let i = 0; i < batch.length; i++) {
			...
			low.append({
				...
				imu_b64: pkt.b64,
				...(typeof fatigueValue === 'number' ? { fatigue: fatigueValue } : {}),
			});
		}
	}

	function onBatchA(batch?: RawPacket[] | null, fatigueValue?: number | null) {
		appendRows('A', idA, batch, fatigueValue);
	}

	function onBatchB(batch?: RawPacket[] | null, fatigueValue?: number | null) {
		appendRows('B', idB, batch, fatigueValue);
	}
