...
export function DualImuProvider({ entryA, entryB, expectedHzA, expectedHzB, onBatchA, onBatchB, autoStart = true, collect = false, children }: Props) {
	...
	const value = useMemo<Ctx>(() => {
		...
		return {
			...
			setCollectAll: (v: boolean) => {
				if (hasA) a.setCollect(v);
				if (hasB) b.setCollect(v);
			},
			resetStatsAll: () => {
				if (hasA) a.resetStats();
				if (hasB) b.resetStats();
			},
